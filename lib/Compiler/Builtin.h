...
export static const auto PROSPECT_TABLE_ABS=auto[526](
...
k[i]=dot(lerp(PROSPECT_TABLE_ABS[w0],PROSPECT_TABLE_ABS[w0+1],w),contents)-xanthophylls*lerp(PROSPECT_CX_TABLE[x0],PROSPECT_CX_TABLE[x0+1],x);
}
...
absorption[i]=lerp(MARMIT_TABLE_ABS[w0],MARMIT_TABLE_ABS[w0+1],w);
}
const auto s=2.*saturate((color(&$state.wavelength_base[0])-MARMIT_MIN_WAVELENGTH)/(MARMIT_MAX_WAVELENGTH-MARMIT_MIN_WAVELENGTH))-1.;
