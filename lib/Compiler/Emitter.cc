...
void Emitter::declare_parameter(const Parameter &param, Value value) {
  ...
  declare_crumb(param.name, /*decl=*/{}, value);
  if (param.is_inline())
...
void Emitter::unwind(Crumb *lastCrumb) {
  ...
  for (; crumb && crumb != lastCrumb; crumb = crumb->prev) {
    if (crumb->is_ast_defer()) {
      ...
      builder.CreateStore(crumb->preservedValue, crumb->value);
    } else if (crumb->value) {
...
Value Emitter::create_result(Type *type, llvm::ArrayRef<Result> results,
  ...
  if (type->is_abstract()) {
    ...
    if (context.get_conversion_rule(resultType, type) ==
      ...
      srcLoc.throw_error(
          concat("inferred result type ", quoted(resultType->displayName),
...
Value Emitter::emit_op(AST::UnaryOp op, Value value,
  ...
  if (value.is_comptime_meta_type(context)) {
    ...
    case UNOP_DEREF:
      ...
      srcLoc.throw_error(
          concat("cannot dereference ", quoted(type->displayName)));
      break;
...
Value Emitter::emit_op(AST::BinaryOp op, Value lhs, Value rhs,
  ...
  if ((op & BINOP_EQ) == BINOP_EQ) {
    if (!lhs.is_lvalue())
      srcLoc.throw_error(
          concat("cannot apply ", quoted(to_string(op)), " to rvalue"));
    if (op != BINOP_EQ)
...
Value Emitter::emit_intrinsic(std::string_view name, const ArgumentList &args,
  ...
  auto expectOne{[&]() {
    if (args.size() != 1)
      srcLoc.throw_error(
          concat("intrinsic ", quoted(name), " expects 1 argument"));
    return args[0].value;
  }};
  auto expectOneVectorized{[&]() {
    if (args.size() != 1 || !args[0].value.type->is_vectorized())
      srcLoc.throw_error(
          concat("intrinsic ", quoted(name), " expects 1 vectorized argument"));
    return args[0].value;
  }};
...
Value Emitter::resolve_identifier(Span<std::string_view> names,
  ...
  srcLoc.throw_error(
      concat("cannot resolve identifier ", quoted(join(names, "::"))));
  return Value();
}
  ...
  for (size_t paramIndex = 0; paramIndex < params.size(); paramIndex++) {
      ...
      for (size_t argIndex = 0; argIndex < args.size(); argIndex++) {
        if (auto &arg{args[argIndex]}; arg.name == param.name) {
          ...
          if (isResolved(argIndex))
            srcLoc.throw_error(
                concat("named argument ", quoted(arg.name),
    ...
    if (arg) {
      ...
      if (!context.is_implicitly_convertible(arg->value.type, param.type))
        srcLoc.throw_error( //
            concat("argument type ", quoted(arg->value.type->displayName),
      ...
      srcLoc.throw_error( //
          concat("missing argument for parameter ", quoted(param.name),
