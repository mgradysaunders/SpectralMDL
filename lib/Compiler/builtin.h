...
@(hot noinline) color rgb_to_color_nontrivial(float3 rgb) {
  ...
  w *= RGB_TO_COLOR_NUM_WAVELENGTHS / 
      (RGB_TO_COLOR_MAX_WAVELENGTH - RGB_TO_COLOR_MIN_WAVELENGTH);
  for (int i = 0; i < $WAVELENGTH_BASE_MAX; i++) {
    ...
    if ((0.0 <= t) & (t <= RGB_TO_COLOR_NUM_WAVELENGTHS)) {
      ...
      c[i] = #sum(float2(1 - t, t) * 
            (coeff_w * float2(&RGB_TO_COLOR_CURVES[0][t0]) + 
...
export enum intensity_mode {
  /// Power (watts) per unit area (meters squared).
...
/// An mat of a material corresponding to `smdl::JIT::Material::Instance` 
/// in the C++ API.
export struct $material_instance {
  ...
  const &color absorption_coefficient = &mat.volume.absorption_coefficient;

  /// The volume scattering coefficient.
  const &color scattering_coefficient = &mat.volume.scattering_coefficient;
