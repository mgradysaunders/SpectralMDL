...
export @(pure macro) auto tint(const color_or_float tint, const bsdf base) = tint1_t(tint, base);
export @(pure macro) auto tint(const color_or_float tint, const edf base) = tint1_t(tint, base);
export @(pure macro) auto tint(const color_or_float tint, const hair_bsdf base) = tint1_t(tint, base);
export @(pure macro) auto tint(const color_or_float reflection_tint, const color_or_float transmission_tint, const bsdf base) = tint2_t(reflection_tint, transmission_tint, base);
@(macro)
auto evaluate_bsdf(const &tint1_t this, const &evaluate_bsdf_parameters params) {
  auto result(evaluate_bsdf(visit &this.base, params));
  if (!result.is_black) result.f *= this.tint;
  return result;
}
@(macro)
auto evaluate_bsdf(const &tint2_t this, const &evaluate_bsdf_parameters params) {
  auto result(evaluate_bsdf(visit &this.base, params));
  if (!result.is_black) {
...
@(macro)
auto evaluate_bsdf_sample(const &tint1_t this, const &evaluate_bsdf_sample_parameters params) {
  auto result(evaluate_bsdf_sample(visit &this.base, params));
  if ((result.mode != scatter_none) & bool(result.delta_f)) *result.delta_f *= this.tint;
  return result;
}
@(macro)
auto evaluate_bsdf_sample(const &tint2_t this, const &evaluate_bsdf_sample_parameters params) {
  auto result(evaluate_bsdf_sample(visit &this.base, params));
...
struct thin_film_t: bsdf {
  float thickness; 
  color_or_float ior;
  bsdf base;
};
export @(pure macro) auto thin_film(const float thickness, const color_or_float ior, const bsdf base = bsdf()) {
  if (thickness > 0) {
    return thin_film_t(thickness, ior, base);
  } else {
    return base;
  }
}
@(macro)
auto evaluate_bsdf(const &thin_film_t this, const &evaluate_bsdf_parameters params) {
  return evaluate_bsdf(visit &this.base, params);
}
@(macro)
auto evaluate_bsdf_sample(const &thin_film_t this, const &evaluate_bsdf_sample_parameters params) {
  return evaluate_bsdf_sample(visit &this.base, params);
}
tag factor;
export struct fresnel_factor: bsdf, factor {
  color ior;
  color extinction_coefficient;
  bsdf base = bsdf();
};
export struct directional_factor: bsdf, factor {
  color_or_float normal_tint = 1.0;
  color_or_float grazing_tint = 1.0;
  float exponent = 5.0;
  bsdf base = bsdf();
};
export struct measured_factor: bsdf, factor {
  texture_2d values;
  bsdf base = bsdf();
};
export struct measured_curve_factor: bsdf, factor {
  color[<N>] curve_values;
  bsdf base = bsdf();
};
@(pure)
auto evaluate_factor(const &fresnel_factor this, const float3 wo, const float3 wi) {
  return 1.0;
}
@(pure)
auto evaluate_factor(const &directional_factor this, const float3 wo, const float3 wi) {
  return schlick_fresnel(dot(wo, normalize(wo + wi)), this.normal_tint, this.grazing_tint, this.exponent);
}
@(pure) 
auto evaluate_factor(const &measured_factor this, const float3 wo, const float3 wi) {
  const auto wh(normalize(wo + wi));
  const auto u(saturate(#acos(dot(wo, wh)) / $HALF_PI));
  const auto v(saturate(#acos(wh.z) / $HALF_PI));
  return tex::lookup_color(this.values, float2(u, v), tex::wrap_clamp, tex::wrap_clamp);
}
@(pure)
auto evaluate_factor(const &measured_curve_factor this, const float3 wo, const float3 wi) {
  const auto t(saturate(#acos(dot(wo, normalize(wo + wi))) / $HALF_PI));
  const auto t0(clamp(int(t * this.curve_values.size), 0, this.curve_values.size - 1));
  const auto curve0(this.curve_values[t0]);
  const auto curve1(this.curve_values[t0 + 1]);
  return lerp(curve0, curve1, t - t0);
}
@(macro)
auto evaluate_bsdf(const &factor this, inline const &evaluate_bsdf_parameters params) {
  auto result = evaluate_bsdf(visit &this.base, params);
  if ((mode == scatter_reflect) & !result.is_black) result.f *= evaluate_factor(this, wo, wi);
  return result;
}
@(macro)
auto evaluate_bsdf_sample(const &factor this, inline const &evaluate_bsdf_sample_parameters params) {
  auto result(evaluate_bsdf_sample(visit &this.base, params));
  if ((result.mode == scatter_reflect) & bool(result.delta_f)) *result.delta_f *= evaluate_factor(this, wo, result.wi);
  return result;
}
@(macro)
...
auto evaluate_bsdf(const &weighted_layer this, const &evaluate_bsdf_parameters params) {
  const auto result0(evaluate_bsdf(visit &this.base, params));
  const auto result1(evaluate_bsdf(visit &this.layer, params, this.normal));
  return evaluate_bsdf_result(pdf: lerp(result0.pdf, result1.pdf, this.chance), f: lerp(result0.f, result1.f, this.weight));
}
@(macro)
auto evaluate_bsdf_sample(const &weighted_layer this, inline const &evaluate_bsdf_sample_parameters params) {
  return weighted_bool_sample(&xi.w, this.chance) ? evaluate_bsdf_sample(visit &this.layer, params, this.normal) : evaluate_bsdf_sample(visit &this.base, params);
}
...
auto evaluate_bsdf(const &fresnel_layer this, inline const &evaluate_bsdf_parameters params) {
  ...
  const auto result1(evaluate_bsdf(visit &this.layer, params, this.normal));
  return evaluate_bsdf_result(is_black: true) if (result0.is_black & result1.is_black);
  return evaluate_bsdf_result(
...
auto evaluate_bsdf_sample(const &fresnel_layer this, inline const &evaluate_bsdf_sample_parameters params) {
  ...
  if (weighted_bool_sample(&xi.z, chance)) {
    preserve ior;
    ior = backface ? (1 / this.average_ior) : this.average_ior;
    auto result(evaluate_bsdf_sample(visit &this.layer, params, this.normal));
...
tag curve_layer;
export struct custom_curve_layer: bsdf, curve_layer {
...
export struct measured_curve_layer: bsdf, curve_layer {
...
@(macro) 
auto evaluate_bsdf(const &curve_layer this, inline const &evaluate_bsdf_parameters params) {
  return evaluate_bsdf(visit &this.base, params);
  /*
...
@(macro)
auto evaluate_bsdf_sample(const &curve_layer this, inline const &evaluate_bsdf_sample_parameters params) {
  return evaluate_bsdf_sample(visit &this.base, params);
  /*
...
export @(macro) int material__evaluate_bsdf(
  ...
  auto result(return_from {
    if (#typeof(this.backface) != #typeof(material_surface()) && backface) 
      return evaluate_bsdf(visit &this.backface.scattering, &params);
    else
      return evaluate_bsdf(visit &this.surface.scattering, &params);
  });
  if (result.is_black) {
    *pdf_fwd = 0.0;
    *pdf_rev = 0.0;
...
export @(macro) int material__evaluate_bsdf_sample(
  ...
  auto sample_result(return_from {
    if (#typeof(this.backface) != #typeof(material_surface()) && backface)
      return evaluate_bsdf_sample(visit &this.backface.scattering, &sample_params);
    else
      return evaluate_bsdf_sample(visit &this.surface.scattering, &sample_params);
  });
