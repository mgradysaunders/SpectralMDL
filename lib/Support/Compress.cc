...
std::vector<std::byte> compressBytes(Span<const std::byte> bytes, int level) {
  ...
  if (status != MZ_OK)
    throw Error(concat("Cannot deflate ", Bytes(bytes.size()), ": ",
                       mz_error(status)));
...
void decompressBytesInto(Span<std::byte> dest, Span<const std::byte> bytes) {
  ...
  if (bytes.empty())
    throw Error("Cannot inflate an empty stream");
  const int status{
      mz_uncompress(reinterpret_cast<unsigned char *>(dest.data()), &destSize,
                    reinterpret_cast<const unsigned char *>(bytes.data()),
                    mz_ulong(bytes.size()))};
  if (status != MZ_OK)
    throw Error(concat("Cannot inflate ", Bytes(bytes.size()), ": ",
                       mz_error(status)));
  if (destSize != dest.size())
    throw Error(concat("Cannot inflate ", Bytes(bytes.size()), ": it holds ",
                       Bytes(destSize), " where ", Bytes(dest.size()),
                       " were expected"));
}
