...
" first, with exactly one `stop` among them, and any media of its own that
...
" for a Sellmeier `c`, a squared wavelength in square micrometers as a
" datasheet prints it.
"
...
" The medium's own: a word after `medium` that is no medium name. After a
" definition's `medium` it is still followed into the block, so the settings
" there keep their meaning.
syn match lensMediumBadName contained "[^ \t{}=#]\+"
      \ nextgroup=lensMediumBlock skipwhite skipempty
"--}
...
" them, front first and film last; and the media of its own that they name,
" `medium NAME { ... }`, above or below the first surface that names one.
...
syn keyword lensSetting contained medium
      \ nextgroup=lensMediumDefinedName,lensMediumBadName skipwhite skipempty

" A medium name is a letter, then letters, digits, `-`, and `_`, which is how
" makers spell a designation (N-BK7), and it matches ignoring case. A word
" with any other character in it is no name at all.
syn match lensMediumDefinedName contained "\a[-0-9A-Za-z_]*[^ \t{}=#]\@!"
      \ nextgroup=lensMediumBlock skipwhite skipempty
...
" wavelength, and defaults to air; `medium` names the medium that space is
...
syn keyword lensSurfaceSetting contained medium
      \ nextgroup=lensMediumName,lensMediumBadName skipwhite skipempty
syn match lensMediumName contained "\a[-0-9A-Za-z_]*[^ \t{}=#]\@!"
...
" A medium the lens defines, by one of three methods: `ior` and `abbe`, the
...
syn keyword lensMediumSetting contained ior abbe partial_dispersion
syn keyword lensMediumSetting contained sellmeier
...
syn region lensMediumBlock contained matchgroup=lensDelim start="{" end="}"
      \ contains=@lensCommon,lensMediumSetting
...
" Blocks nest at most three deep (lens, medium, sellmeier) and are short.
...
hi def link lensMediumSetting     Label
hi def link lensSellmeierRow     Keyword
hi def link lensMediumName        Identifier
hi def link lensMediumDefinedName Identifier
...
if !exists("g:lens_no_error_highlight")
  ...
  hi def link lensMediumBadName   Error
endif
"--}
