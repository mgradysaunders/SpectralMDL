...
TEST_CASE("Motion: the decomposition reassembles the transform") {
  ...
  for (const auto &entry : keys) {
    ...
    CHECK(nearMatrix(composeTransform(parts), entry.second, 2.0e-6f));
...
TEST_CASE("Motion: the ends of an interpolation are the keys themselves") {
  ...
  CHECK(sameMatrix(interpolateTransform(a, b, 0.0f), a));
  CHECK(sameMatrix(interpolateTransform(a, b, 1.0f), b));
  CHECK(sameMatrix(interpolateTransform(a, b, -1.0f), a));
  CHECK(sameMatrix(interpolateTransform(a, b, 2.0f), b));
}
...
TEST_CASE("Motion: a turn slerps, so halfway through 90 degrees is 45") {
  ...
  CHECK(nearMatrix(half, rotationZ(45.0f), 1.0e-5f));
...
TEST_CASE("MotionTrack: an empty track says nothing") {
  ...
  CHECK(track.empty());
  CHECK(sameMatrix(track.at(0.0f), float4x4(1.0f)));
...
TEST_CASE("MotionTrack: a key's own time reproduces the key") {
  ...
  for (const auto &key : track.keys)
    CHECK(sameMatrix(track.at(key.time), key.transform));
}
...
TEST_CASE("MotionTrack: outside the keys the track clamps") {
  ...
  CHECK(sameMatrix(track.at(-5.0f), track.keys.front().transform));
  CHECK(sameMatrix(track.at(0.999f), track.keys.front().transform));
  CHECK(sameMatrix(track.at(2.001f), track.keys.back().transform));
  CHECK(sameMatrix(track.at(100.0f), track.keys.back().transform));
}
...
TEST_CASE("MotionTrack: one key is a constant") {
  ...
  CHECK(sameMatrix(track.at(-1.0f), track.keys[0].transform));
  CHECK(sameMatrix(track.at(7.0f), track.keys[0].transform));
  CHECK(sameMatrix(track.at(99.0f), track.keys[0].transform));
}
