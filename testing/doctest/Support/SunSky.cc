...
TEST_CASE("SunSky") {
  SUBCASE("sky radiance golden") {
    ...
    for (size_t c = 0; c < kGoldenCaseCount; c++) {
      const double sunAzimuthDeg{25.0};
      smdl::SunSkyOptions options{};
      options.sunDirection = makeDirection(kSkyCases[c][0], sunAzimuthDeg);
      options.visibility = float(kSkyCases[c][3]);
      options.waterVaporScale = float(kSkyCases[c][4]);
      options.scaleFactor = 0.1f;
      const auto sunSky{smdl::SunSky(options)};
      const auto direction{
          makeDirection(kSkyCases[c][1], sunAzimuthDeg + kSkyCases[c][2])};
      std::vector<float> wavelens(kGoldenChannelCount);
      std::vector<float> radiance(kGoldenChannelCount);
      for (size_t j = 0; j < kGoldenChannelCount; j++)
        wavelens[j] = channelWavelength(kGoldenChannels[j]);
      sunSky.skyRadiance(direction, int(kGoldenChannelCount), wavelens.data(),
                         radiance.data());
      float peak{0.0f};
      for (size_t j = 0; j < kGoldenChannelCount; j++)
        peak = std::max(peak, kSkyExpected[c][j]);
      float worst{0.0f};
      for (size_t j = 0; j < kGoldenChannelCount; j++)
        worst =
            std::max(worst, std::abs(radiance[j] - kSkyExpected[c][j]) / peak);
      CAPTURE(c);
      CHECK(worst < 5e-4f);
    }
  }
  SUBCASE("sun radiance golden") {
    ...
    for (size_t c = 0; c < kGoldenCaseCount; c++) {
      smdl::SunSkyOptions options{};
      options.sunDirection = makeDirection(kSunCases[c][0], 0.0);
      options.visibility = float(kSunCases[c][1]);
      options.waterVaporScale = float(kSunCases[c][2]);
      options.scaleFactor = 0.1f;
      const auto sunSky{smdl::SunSky(options)};
      std::vector<float> wavelens(kGoldenChannelCount);
      std::vector<float> radiance(kGoldenChannelCount);
      for (size_t j = 0; j < kGoldenChannelCount; j++)
        wavelens[j] = channelWavelength(kGoldenChannels[j]);
      sunSky.sunRadiance(int(kGoldenChannelCount), wavelens.data(),
                         radiance.data());
      float peak{0.0f};
      for (size_t j = 0; j < kGoldenChannelCount; j++)
        peak = std::max(peak, kSunExpected[c][j]);
      float worst{0.0f};
      for (size_t j = 0; j < kGoldenChannelCount; j++)
