...
TEST_CASE("VectorMath: transforming points and directions") {
  SUBCASE("transformPoint carries the translation") {
    const float4x4 xf{awkwardTransform()};
    const float3 origin{smdl::transformPoint(xf, float3(0.0f, 0.0f, 0.0f))};
    ...
    const float3 point{smdl::transformPoint(xf, float3(1.0f, 1.0f, 1.0f))};
  ...
  SUBCASE("transformDirection drops the translation") {
    const float4x4 xf{awkwardTransform()};
    const float3 zero{smdl::transformDirection(xf, float3(0.0f, 0.0f, 0.0f))};
    CHECK(zero.x == 0.0f);
    CHECK(zero.y == 0.0f);
    CHECK(zero.z == 0.0f);
    ...
    const float3 direct{smdl::transformDirection(xf, to - from)};
    const float3 viaPoints{smdl::transformPoint(xf, to) -
  ...
  SUBCASE("A rigid transform round-trips through its affine inverse") {
    const float4x4 rigid{smdl::lookAt(float3(3.0f, -1.0f, 2.0f), float3(0.0f))};
    const float4x4 inverse{smdl::affineInverse(rigid)};
  ...
  SUBCASE("A pure translation moves points and leaves directions alone") {
    ...
    const float3 point{smdl::transformPoint(shift, float3(1, 1, 1))};
    const float3 direction{smdl::transformDirection(shift, float3(1, 1, 1))};
    CHECK(point.x == 5.0f);
    CHECK(point.y == 6.0f);
    CHECK(point.z == 7.0f);
...
TEST_CASE("VectorMath: the three by three inverse") {
  ...
  SUBCASE("A matrix times its inverse is the identity") {
    ...
    const double3x3 product{m * inverse};
  ...
  SUBCASE("A singular one is left alone") {
    ...
    const double3x3 before{singular};
