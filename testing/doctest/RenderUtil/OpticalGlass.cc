...
// A Sellmeier fit with each C a wavelength in micrometers, as Malitson and
// Dodge print it.
[[nodiscard]] double sellmeierOfWavelengths(const double (&b)[3],
                                            const double (&c)[3],
                                            double wavelength) {
...
TEST_CASE("OpticalGlass: the built-in catalog") {
  ...
  SUBCASE("Fused silica and the fluorides are Malitson's and Dodge's fits, "
          "whose C they print as a wavelength") {
    ...
    constexpr double MGF2_B[3]{0.48755108, 0.39875031, 2.3120353};
    constexpr double MGF2_C[3]{0.04338408, 0.09461442, 23.793604};
    const smdl::OpticalGlass &silica{catalogGlass("FUSED-SILICA")};
    const smdl::OpticalGlass &fluorite{catalogGlass("CAF2")};
    const smdl::OpticalGlass &magnesium{catalogGlass("MGF2")};
    for (int wavelength = 300; wavelength <= 2500; wavelength += 100) {
      CHECK_NEAR(silica.indexAt(float(wavelength)),
                 sellmeierOfWavelengths(SILICA_B, SILICA_C, wavelength), 1e-6);
      CHECK_NEAR(fluorite.indexAt(float(wavelength)),
                 sellmeierOfWavelengths(FLUORITE_B, FLUORITE_C, wavelength),
                 1e-6);
      CHECK_NEAR(magnesium.indexAt(float(wavelength)),
                 sellmeierOfWavelengths(MGF2_B, MGF2_C, wavelength), 1e-6);
    }
  }
