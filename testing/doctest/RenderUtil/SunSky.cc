...
TEST_CASE("SunSky: the fitted radiances against their generator") {
  ...
  SUBCASE("The sun disk over a resolved grid is what the disk adds to the "
          "sky") {
    smdl::SunSkyOptions options{};
    options.sunDirection = makeDirection(40.0, 25.0);
    options.scaleFactor = 3.0f;
    const smdl::SunSky sunSky{options};
    // Between channels, on one, and past both ends of the model's range.
    const float wavelens[5] = {380.0f, 452.5f, 550.0f, 1601.0f, 2600.0f};
    smdl::SkyBasis basis{};
    sunSky.resolve(smdl::Span<const float>(wavelens, 5), basis);
    float resolved[5]{}, direct[5]{};
    sunSky.sunRadiance(basis, resolved);
    sunSky.sunRadiance(5, wavelens, direct);
    float inside[5]{}, sky[5]{};
    sunSky.radiance(sunSky.sunDirection(), basis, inside);
    sunSky.skyRadiance(sunSky.sunDirection(), basis, sky);
    for (int j = 0; j < 5; j++) {
      CHECK(resolved[j] > 0.0f);
      CHECK(resolved[j] == direct[j]);
      CHECK(resolved[j] == doctest::Approx(inside[j] - sky[j]).epsilon(1e-5));
    }
    options.isSunEnabled = false;
    const smdl::SunSky skyOnly{options};
    skyOnly.resolve(smdl::Span<const float>(wavelens, 5), basis);
    skyOnly.sunRadiance(basis, resolved);
    for (int j = 0; j < 5; j++) CHECK(resolved[j] == 0.0f);
  }
