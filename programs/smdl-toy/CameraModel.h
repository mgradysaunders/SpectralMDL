...
struct CameraModel final {
  ...
  ///
  /// \{
  CameraDocument cameraDoc{};
  LayoutDiagnostics cameraDocDiags{};
  /// \}

  /// The associated camera, sensor, and lens file names, if applicable. These
  /// may all be empty, indicating everything was set from the command line.
  /// Kept around only for the log and report.
  ///
  /// \{
  ...
  [[nodiscard]] bool shouldApproximateLens() const noexcept {
    return isPreview && cameraOptions.lens.has_value();
  }
};

/// The name of a film quantity as the headers spell it.
[[nodiscard]] inline const char *
filmQuantityName(FilmQuantity quantity) noexcept {
...
thinLensFocalLength(const CameraOptions &options) noexcept {
...
///
[[nodiscard]] CameraModel resolveCameraModel(const Options &options);
