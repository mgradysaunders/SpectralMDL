...
/// A mesh instance whose material has a non-default emission EDF.
class AreaLight final {
public:
  /// The index in the `Scene::meshInstances` array.
  uint32_t meshInstanceIndex{INVALID_INDEX};

  /// The area-weighted distribution over the mesh faces.
  smdl::Distribution1D faceDistr{};

  /// The total surface area, which is also the reciprocal of the uniform
  /// area sampling density, and the divisor `intensity_power` emission
  /// requires the host to apply.
  float totalArea{};
};
