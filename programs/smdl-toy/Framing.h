...
/// What `solveFraming()` decided, plus the diagnostics that make a batch
/// run's choices auditable.
struct FramingResult final {
  ...
  /// which is what the azimuth sweep maximizes; see `solveFraming()`.
  float visibleArea{};
...
/// area; see `ProbeResult` in `Framing.cc`. With it set, the direction is
...
///
[[nodiscard]] FramingResult solveFraming(const Scene &scene,
                                         const FramingOptions &options);
