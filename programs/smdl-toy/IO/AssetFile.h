...
// Only the shared vocabulary, never "Scene/Scene.h": keeping this header
// free of Embree and compiler dependencies lets the layout loader read
...
/// An `.asset` manifest in memory: the mesh file to render, plus the
/// correction from the space it was authored in into the space a
/// composition places it in.
///
...
///
[[nodiscard]] AssetFile readAssetFile(const std::string &fileName);
