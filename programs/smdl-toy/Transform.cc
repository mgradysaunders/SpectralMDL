...
// The transform decomposition and the blend between two transforms.
// Both the layout's `motion` blocks, the rigid `.places` record, and the
// Embree hand-off in `Scene.cc` run through `decomposeTransform()`, so
// what a file means by a key and what the traversal does between two of
// them are one procedure.
