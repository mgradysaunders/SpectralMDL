/// \file
/// The contexts a path is traced in: what the render fixes for every
/// path it traces (`RenderContext`, with the bounds and estimators the
/// command line chose) and what one path carries (`PathContext`). The
/// path walk and every visibility walk it spawns run in these, and
/// nothing here knows how a path is traced.
#pragma once
